...
public class CreateCode {
    ...
    //获取二维码的流对象（有底部文字，中间logo）
    public BufferedImage createCodeImageToStream(CodeModel info) {
        ...
        if (!ObjectUtil.isEmpty(desc)) {
            int whiteWidth;
            if (info.getWhiteWidth() == null) {
                whiteWidth = info.getHeight() - info.getBottomEnd()[1];

            } else {
                whiteWidth = info.getWhiteWidth();
            }
            Font font = new Font("Courier", Font.CENTER_BASELINE, info.getFontSize());
    ...
    public void createCodeImage(CodeModel info, OutputStream output) {
        ...
        try {
            ImageIO.write(bm, ObjectUtil.isEmpty(info.getFormat()) ? info.getFormat() : info.getFormat(), output);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void main(String[] args) {
        ...
        info.setHeight(160);
        info.setFontSize(12);
        info.setWhiteWidth(8);
        info.setContents("万水千山只等闲\n小荷才楼尖尖角.你怎么会知道我的苦");
        info.setLogoFile(null);
        info.setDesc("         NO." + "25fdc152");
        createCode.createCodeImage(info, "E:\\2Dcode\\123." + info.getFormat());
    }
}
